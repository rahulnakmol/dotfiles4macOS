...
| Machine-local shared gateway | `~/.config/private-ai-gateway/{endpoint,client.key,default-model}` | One endpoint, one mode-0600 key and selected model for Claude Code, Codex and OpenCode |
...
| Machine-local launchers | `~/.local/bin/{codex,chatgpt-subscription,chatgpt-aigateway,codex-profile}` | Gateway-only CLI route and explicit desktop routes |
...
authenticated `/v1/models`, and selects the default Codex/OpenCode model. It installs immutable
