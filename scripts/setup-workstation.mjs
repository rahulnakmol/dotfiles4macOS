...
import { resolveProfile, guidedApps } from './workstation-profiles.mjs';
...
export function applyWorkstation(id, home, backup, source=root) {
  const config=resolveProfile(id), target=destination(home,id), files=workstationFiles(id);
  ...
  records.push({path:join(target,'managed-files.json'),before:presentFile(join(target,'managed-files.json')),after:Buffer.from(JSON.stringify(Object.fromEntries([...files].map(([p,b])=>[p,hash(b)])),null,2)+'\n').toString('base64')});
  const selector={profile:id,source:resolve(source)};
  records.push({path:statePath(home),before:presentFile(statePath(home)),after:Buffer.from(JSON.stringify(selector,null,2)+'\n').toString('base64')});
  const journal={version:1,profile:id,source:resolve(source),home:resolve(home),target,links:plan.links.filter(l=>l.change),records,status:'applying'};
  ...
  const modules=[...config.install.modules,'alfred','karabiner','rectangle-pro'];
  try {
...
export function undoWorkstation(backup, home, source=root) {
  ...
  const allowed=new Set([...workstationFiles(j.profile).keys()].map(p=>join(j.target,p)));
  allowed.add(join(j.target,'managed-files.json'));allowed.add(statePath(home));
  const linkAllowed=new Map(desiredLinks(workstationFiles(j.profile),j.target,home).map(l=>[l.path,l.target]));
...
export function packagePlan(config, installedFormulae, installedCasks, detect=appInstalled, detectCommand=command=>spawnSync('which',[command]).status===0) {
  return {
    ...
    guided:guidedApps.filter(a=>!detect(a)),
  };
}

function packages(config) {
  if(spawnSync('which',['brew']).status!==0)return {formulae:config.install.formulae,casks:config.install.casks.map(a=>a.cask),guided:guidedApps.filter(a=>!appInstalled(a)),homebrewMissing:true};
  return packagePlan(config,run('brew',['list','--formula','-1']).trim().split('\n'),run('brew',['list','--cask','-1']).trim().split('\n'));
}
function machineReport(config,target,home) {
  let location=null;
...
async function main() {
  ...
  if(args.some((a,i)=>!(a==='--profile'||i===index+1&&index>=0)))throw new Error('Only --profile fde|tf is supported');
  ...
  const config=resolveProfile(id);id=config.setupProfile.toLowerCase();
  const major=Number(run('sw_vers',['-productVersion']).trim().split('.')[0]);
  if(major<26)throw new Error('These profiles require macOS 26+ for the native Amp app. No configuration changed.');
  const target=destination(home,id),files=workstationFiles(id),plan=linkPlan(files,target,home);
  for(const path of files.keys())assertLocalPath(join(target,path),home);
  assertLocalPath(join(target,'managed-files.json'),home);assertLocalPath(statePath(home),home);
  ...
  const report={mode,profile:config.setupProfile,target,conflicts:[...plan.conflicts,...drift],linksToChange:plan.links.filter(l=>l.change).map(l=>l.path),missingFiles:missingFiles.length,outdatedFiles:outdated,install,...machine};
  ...
  if(mode==='apply') {
    ...
    applyWorkstation(id,home,backup);
    console.log(`Applied ${config.setupProfile}. Backup: ${backup}\nRollback: bash scripts/setup-workstation.sh rollback '${backup}'\nComplete the guided steps in docs/profiles/${id}.html, then rerun apply/check after connecting Alfred.`);
  }
  if(mode==='check') {
    const pending=report.conflicts.length||report.linksToChange.length||missingFiles.length||outdated.length||install.formulae.length||install.casks.length||install.guided.length||!machine.alfredConnected||machine.alfredPreferencesPending||machine.missingWorkflows.length||machine.pendingPresets.length;
