...
const focusRows = config.focusSessions.map((s) => `<tr><td><code>fs ${escape(s.id)}</code></td><td>Focus Session: ${escape(s.name)}</td><td>${escape(s.apps.map(appName).join(' + '))}</td><td>${s.id === 'work' ? 'Edge ⅔ · Teams ⅓' : 'Desktop 1: first app ⅔ · Ghostty ⅓<br>Desktop 2: chosen coding app maximized'}</td></tr>`).join('');
...
${mdTable(['Alfred command','Session','Apps'],config.focusSessions.map((s)=>['fs '+s.id,'Focus Session: '+s.name,s.apps.map(appName).join(' + ')]))}
