...
const codexCommands = { 'openAvatarOverlay': 'Toggle Codex pet · global while app runs', 'composer.startVoiceMode': 'Toggle voice chat · Codex app', 'composer.startDictation': 'Start dictation · Codex app' };
...
const focusRows = config.focusSessions.map((s) => `<tr><td><code>fs ${escape(s.id)}</code></td><td>Focus Session: ${escape(s.name)}<br>${s.durationMinutes} minutes in Session</td><td>${escape(s.apps.map(appName).join(' + '))}</td><td>${s.id === 'work' ? 'Edge ⅔ · Teams ⅓' : 'Desktop 1: Zen maximized<br>Desktop 2: coding app maximized<br>Desktop 3: Ghostty ⅔ · Slack ⅓'}</td></tr>`).join('');
...
<div class="two"><div><h3>Apps</h3><div class="table-scroll"><table class="key-table"><thead><tr><th>Shortcut</th><th>App</th><th>Action</th></tr></thead><tbody>${appRows}</tbody></table></div><p class="muted" style="margin-top:16px;font-size:13px">A/S/D/F = Amp, Slack, Zen Browser, Finder. H/J/K/L = Ghostty, Codex, Cursor, Claude. W/E = Word/Edge; I/O/P = Teams/Excel/PowerPoint. R = Safari; N = Obsidian notes. Z/X/C = Final Cut Pro, Motion, Compressor: edit → animate → export. Creative app keys work where installed; no apps are installed by this setup. One direct key per app. On this Mac, ChatGPT is the installed Codex host. Letters prioritize apps and voice; punctuation and arrows control windows. Native Control+Left/Right switches adjacent desktops.</p></div><div><h3>Navigation & windows</h3><div class="table-scroll"><table class="key-table"><thead><tr><th>Shortcut</th><th>Action</th></tr></thead><tbody>${navRows}${windowRows}${codexRows}</tbody></table></div></div></div>
...
code:[['Desktop 1','Browser','Zen Browser','','Maximized.'],['Desktop 2','Code','Amp / Claude / Cursor / T3 Code','','The selected focus variation maximizes its coding app here.'],['Desktop 3','Terminal + chat','Ghostty','Slack','Ghostty left two-thirds; Slack right third.']],
